...
const ClassEdit = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:8080/groups/get/all')
            .then(res => res.json())
    ...
    const handleGroupDelete = async (key) => {
        const response = await fetch(`http://localhost:8080/groups/delete?groupId=${key}`, {
            method: 'DELETE',
            headers: {
    ...
    const handleModalSubmit = async () => {
        ...
        if (groupData.name.length === 0) {
            ...
            const response = await fetch(`http://localhost:8080/groups/new`, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
        ...
        function handleTimeChange(value) {
            setGroupData(prevState => {
                return ({
                    ...prevState,
                    classTime: [value.$d.getHours(), value.$d.getMinutes()]
                })
            })
        ...
        return (
            <div className="ClassEdit">
                <Button onClick={() => setIsModalOpen(true)} style={{marginBottom: 10}} type="primary">Dodaj</Button>
                <Modal
