...
import '../styles/ClassScheduleTile.css'
...
const ClassScheduleTile = (props) => {
    ...
    const handleClimberDelete = async (value) =>{
        const res = await  fetch(`http://localhost:8080/groups/delete-climber?climberId=${value}&groupId=${props.data.id}`,
            {
                method: 'PATCH',
    ...
    const onClimberSearch = async (value) =>{
        ...
        setSearchValue('')
        // search for climber
        const response = await fetch(`http://localhost:8080/climbers/get/by-card-number?cardNumber=${value}`, {
            method: 'GET',
            headers: {
        ...
        const res = await  fetch(`http://localhost:8080/groups/add-climber?climberId=${climberId}&groupId=${props.data.id}`,
            {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' }
            })
    ...
    function handleRedirect(cardNumber){
        navigate('/Profil',{state:{cardNumber: cardNumber}});
    }
    ...
    return(
        <div>
            <div className={`Tile ${groupAdvancementClassName}`} onClick={()=>setIsModalOpen(true)}>
                <h1>{data.coach}</h1>
...
export default ClassScheduleTile